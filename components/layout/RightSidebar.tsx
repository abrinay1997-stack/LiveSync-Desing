    ...
    return (
        <div className="absolute top-12 right-0 bottom-0 w-80 bg-[#09090b] border-l border-white/5 flex flex-col z-40 pointer-events-auto">
            ...
            <div className="flex border-b border-white/5 bg-[#18181b]">
                ...
                <button
                    onClick={() => setRightTab('patch')}
                    className={`flex-1 py-2 text-xs font-medium transition-all ${ui.activeRightTab === 'patch' ? 'text-aether-accent border-b-2 border-aether-accent bg-white/5' : 'text-gray-500 hover:text-gray-300'}`}
                >
                    Patch
                </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar bg-[#09090b]">
                {ui.activeRightTab === 'inspector' && <PropertiesPanel />}
                {ui.activeRightTab === 'layers' && <LayersPanel />}
                {ui.activeRightTab === 'patch' && (
                    <>
                        <ConnectionPanel />
                        <SystemMonitorPanel />
                    </>
